...
describe("editor do agente sem número conectado", () => {
  ...
  it("NÃO deixa publicar sem número, e a dica diz que o rascunho está salvo", () => {
    ...
    // Mesma régua do caso "explica o que falta sem acusar erro": publicar sem número
    // é um passo que falta, não um erro — e o motivo aparece também a cada tecla
    // enquanto há alteração não salva. Vermelho ali acusaria quem não errou.
    expect(screen.getByRole("status").className).not.toMatch(/destructive/);
